    ...
    public class GestionJeu
    {
        ...
        private void clavierKeyDown(object sender, KeyboardKeyEventArgs e)
        {
            switch (EtatDuJeu)
            {
                ...
                case EtatJeu.EnJeu:
                    if (e.Key == Key.Space) balleJeu.Lancer();
                    else if (e.Key == Key.P) MettreEnPause();
                    else if (e.Key == Key.Escape) EtatDuJeu = EtatJeu.Accueil;
                    break;

                case EtatJeu.Pause:
                    if (e.Key == Key.P) MettreEnPause();
                    else if (e.Key == Key.Escape) EtatDuJeu = EtatJeu.Accueil;
                    break;
        ...
        private void GenererBriquesDemo()
        {
            viderListesBriques();

            float centreX = LargeurJeu / 2.0f;
            float centreY = HauteurJeu / 2.0f - 100.0f;
            float largeurBrique = 80.0f;
            float hauteurBrique = 25.0f;

            ajouterBriqueCassable(centreX - 180, centreY, largeurBrique, hauteurBrique, 1, TypeBrique.Normale);
            ajouterBriqueCassable(centreX - 60, centreY, largeurBrique, hauteurBrique, 2, TypeBrique.Normale);
            ajouterBriqueCassable(centreX + 60, centreY, largeurBrique, hauteurBrique, 3, TypeBrique.Dynamique);
            ajouterBriqueIndestructible(centreX + 180, centreY, largeurBrique, hauteurBrique);
        }
